...
func TestScheduleFollowsSM2StyleIntervals(t *testing.T) {
	...
	item, err = progress.Schedule(item, progress.GradeGood, now)
	if err != nil {
	...
	item, err = progress.Schedule(item, progress.GradeGood, item.DueDate.Add(12*time.Hour))
	if err != nil {
	...
	item, err = progress.Schedule(item, progress.GradeEasy, item.DueDate.Add(12*time.Hour))
	if err != nil {
	...
	item, err = progress.Schedule(item, progress.GradeAgain, item.DueDate.Add(12*time.Hour))
	if err != nil {
